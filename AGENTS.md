...
## Testing Guidelines
Backend tests use `pytest` with `pytest-asyncio`; name files `test_*.py` and keep feature-specific coverage close to the service or API under test. Run focused regressions first, for example `python -m pytest backend/tests/test_literature_reader_api.py -q`, then broader suites. For frontend work, prefer live-reload or page-level verification during iterative development instead of forcing a full `npm run build` every time; reserve full builds for release-sensitive changes, shared infra/type changes, explicit user requests, or final pre-commit verification. Keep `npm run lint` as the default lightweight frontend check. Use the PowerShell smoke scripts in `e2e/` when changing auth, role routing, upload, or MCP flows.
