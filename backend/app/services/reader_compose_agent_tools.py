"""
Reader compose agent tool helpers.
"""
...
DEFAULT_READER_AGENT_TOOL_NAMES = {
    "paper_read",
    "knowledge_search",
}


def resolve_reader_agent_tool_whitelist() -> Set[str]:
    raw = str(getattr(settings, "reader_agent_tool_whitelist", "") or "").strip()
    if not raw:
        return set(DEFAULT_READER_AGENT_TOOL_NAMES)
    output: Set[str] = set()
    for token in raw.split(","):
        ...
        if name:
            output.add(name)
    if not output:
        output = set(DEFAULT_READER_AGENT_TOOL_NAMES)
    return output


def build_reader_compose_tool_registry(
    *,
    user_id: int,
    allowed_tool_names: Sequence[str],
) -> ToolRegistry:
    """Build a ToolRegistry for compose-agent usage."""
