...
from app.services.local_structured_pdf.docling_qwen_ocr import log_qwen_ocr_placeholder
...
def _build_ollama_chat_completions_url(base_url: str) -> str:
...
def create_converter(
    ...
    """Create a converter with the specified options.

    This now creates a real Docling `DocumentConverter` instead of routing
    through the local compat adapter.

    OCR is intentionally left on Docling's default pipeline configuration for
    now. The legacy custom Qwen OCR wiring is not migrated in this change.
    """
    ...
    pipeline_options.do_formula_enrichment = bool(enrich_formula)

    qwen_ocr_requested = bool(force_full_page_ocr or ocr_lang)
    if qwen_ocr_requested:
        # Keep the OCR stage on Docling's default configuration for now. The
        # upstream server no longer routes via the compat adapter, but custom
        # Qwen OCR flags are still only logged until the Docling OCR extension
        # point is pinned in this repository.
        log_qwen_ocr_placeholder(
    ...
    if enrich_picture_description:
        ...
        picture_model = os.getenv("LOCAL_STRUCTURED_PDF_HYBRID_MODEL")
        if not ollama_base_url:
            raise RuntimeError(
                "OLLAMA_BASE_URL is required when picture description is enabled."
            )
        if not picture_model:
            raise RuntimeError(
                "LOCAL_STRUCTURED_PDF_HYBRID_MODEL is required when picture description is enabled."
            )
        pipeline_options.do_picture_description = True
        pipeline_options.generate_picture_images = True
        pipeline_options.enable_remote_services = True
        pipeline_options.picture_description_options = PictureDescriptionApiOptions(
            url=_build_ollama_chat_completions_url(ollama_base_url),
            ...
            timeout=float(os.getenv("PDF_HYBRID_BACKEND_PICTURE_TIMEOUT_SECONDS", "60")),
        )
...
def create_app(
    ...
    Args:
        ...
        ocr_lang: Optional OCR language hints reserved for the pending Qwen OCR slot.
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="OCR language hints for the pending Qwen OCR slot (comma-separated, e.g. 'zh,en')",
    )
    parser.add_argument(
        "--enrich-formula",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable picture description model (native Docling VLM path, defaulting to a Qwen VL repo)",
    )
    parser.add_argument(
