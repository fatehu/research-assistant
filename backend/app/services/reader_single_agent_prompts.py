    ...
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "rules": [
            ...
            "Atomic paragraph ownership cannot be split across components (keep source_block_ids ownership intact)",
            ...
            "You may split display paragraphs inside one ParagraphProse via props.paragraphs while keeping source_block_ids unchanged",
            "Use rendered_page_image as soft evidence for internal paragraph boundaries (indent, vertical gap, visible block break)",
            "You may clean obvious OCR/layout noise when it visibly conflicts with the rendered_page_image, especially figure_meta blocks polluted by chart labels or split caption continuation blocks",
            "You may merge caption continuation text into one display caption when the page image and source blocks clearly support it",
            "Do not invent facts, rewrite scientific meaning, or silently drop meaningful source text while cleaning",
            "Do not modify geometry, bbox, polygon, or source ownership while cleaning; preserve all original source_block_ids on the cleaned display component",
            "If image cues indicate multiple paragraphs in one ParagraphProse, prefer props.paragraphs; if uncertain, keep one paragraph and explain in fixes_applied",
            "For FigurePanel.props.image_url, use only renderable image sources: asset:<asset_id>, /api/v1/literature/reader/figure-assets/... , or direct http(s)/data image URLs",
            "Never place DOI/article links or bare file names (e.g., figure_3.png) into FigurePanel.props.image_url",
    ...
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "rules": [
            ...
            "Preserve atomic paragraph ownership (source_block_ids unchanged for same atomic block)",
            ...
            "ParagraphProse may use props.paragraphs for internal paragraph display without changing source_block_ids ownership",
            "Use rendered_page_image as soft evidence for internal paragraph boundaries (indent, vertical gap, visible block break)",
            "You may clean obvious OCR/layout noise when it visibly conflicts with the rendered_page_image, especially figure_meta blocks polluted by chart labels or split caption continuation blocks",
            "You may merge caption continuation text into one display caption when the page image and source blocks clearly support it",
            "Do not invent facts, rewrite scientific meaning, or silently drop meaningful source text while cleaning",
            "Do not modify geometry, bbox, polygon, or source ownership while cleaning; preserve all original source_block_ids on the cleaned display component",
            "When image cues indicate multiple paragraphs in one ParagraphProse node, prefer props.paragraphs while keeping source_block_ids unchanged",
            "For FigurePanel.props.image_url, keep only renderable image sources: asset:<asset_id>, /api/v1/literature/reader/figure-assets/... , or direct http(s)/data image URLs",
            "Do not use DOI/article links or bare file names in FigurePanel.props.image_url",
            "If no reliable figure source can be verified, set image_url='' and record the reason in fixes_applied",
