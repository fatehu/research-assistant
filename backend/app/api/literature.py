...
async def get_citation_graph(
    paper_id: int,
    max_nodes: int = Query(20, ge=5, le=50),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """获取论文引用图谱"""
    ...
    if not paper.semantic_scholar_id:
        raise HTTPException(status_code=400, detail="该论文没有 Semantic Scholar ID，无法获取引用图谱")
    
    service = get_literature_service()
    graph_data = await service.get_paper_network(
        paper.semantic_scholar_id,
        max_per_level=max_nodes // 2
    )
