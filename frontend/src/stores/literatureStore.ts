...
interface LiteratureState {
  ...
  // 图谱
  loadCitationGraph: (paperId: number) => Promise<void>
  clearGraph: () => void
  ...
  // 视图
  setViewMode: (mode: 'list' | 'card' | 'graph') => void
...
export const useLiteratureStore = create<LiteratureState>((set, get) => ({
  ...
  // 图谱
  loadCitationGraph: async (paperId) => {
    set({ graphLoading: true })
    try {
      const graph = await literatureApi.getCitationGraph(paperId, 30)
      set({ citationGraph: graph, graphLoading: false })
    } catch (error) {
