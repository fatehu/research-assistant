...
### 4.4 笔记本共享 (Session 18)

**只读查看模式** - 接收者可以查看笔记本内容，但不能编辑或执行

| 特性 | 说明 |
|------|------|
| **只读模式** | 共享的笔记本仅供查看，无法编辑或执行代码 |
| **完整内容** | 显示所有单元格（代码/Markdown）及其输出结果 |
| **代码高亮** | 代码单元格使用语法高亮显示 |
| **输出展示** | 支持文本输出、执行结果、错误信息、图片显示 |
| **UUID支持** | 笔记本使用UUID作为ID，共享系统完整支持 |

**API端点**：
```
GET  /api/share/my-notebooks              # 获取我的笔记本列表（用于共享）
GET  /api/share/detail/{share_id}         # 获取笔记本完整内容（包括单元格）
POST /api/share/                          # 共享笔记本
```

**数据结构变更**：
- `SharedResource.resource_id` 改为 `VARCHAR(50)` 支持UUID字符串
