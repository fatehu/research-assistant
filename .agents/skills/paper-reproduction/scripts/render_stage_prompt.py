...
def _build_prompt(stage: str, paper_id: int, project_id: int | None, goal: str | None, preferred_draft_id: str | None) -> str:
    ...
    templates = {
        ...
        "grounding": [
            ...
            "grounding 的目标是写出 `specs/grounding_report.json` 作为复现准备清单，而不是尽快把字段填满。",
            "固定顺序：先读 README，再看仓库结构，再合并论文链接与 README/仓库新发现链接，再做轻量探活，最后再决定是否值得进入 implementation。",
            ...
            "如果 `paper_research_probe_url` 返回的是 HTML，不要只看 status_code；先根据 page_title/page_kind/page_signals/suggested_next_action 和页面正文摘要判断它是下载页、landing page、登录页、docs 页还是失效页。",
            ...
            "可以 clone / read / search / inspect / probe，但阶段二先做“值不值得继续”的判断；不要写 implementation_spec，不要写 execution 脚本，不要写 execution_spec，不要 start_execution。",
            "没找到就是 absent 或 blocked，不要把缺失事实翻译成应该存在的脚本、路径或 workaround。",
            "grounding_report 必须明确 blocker、blocker_details、当前可继续部分和 next_actions；如果事实层面卡住，就停在 grounding，不要继续推进。",
        ],
        "implementation_prep": [
