...
def _build_prompt(stage: str, paper_id: int, project_id: int | None, goal: str | None, preferred_draft_id: str | None) -> str:
    ...
    templates = {
        ...
        "grounding": [
            ...
            "grounding 的目标是写出 `specs/grounding_report.json`，把 repo、entrypoint、dataset、runtime、external dependencies 收敛成 grounded/absent/blocked，并明确 repo 主路径的 run decision。",
            "优先使用 `paper_research_probe_repo` 和 `paper_research_probe_url` 做轻量测活，再决定是否 clone repo、读文件、搜 entrypoint 或 inspect runtime。",
            "如果 `paper_research_probe_url` 返回的是 HTML，不要只看 status_code；先根据 page_title/page_kind/page_signals/suggested_next_action 判断具体 blocker，再决定是否继续恢复。",
            "repo 证据一旦足够，调用 `paper_research_assess_repo_mainpath`，明确最可能主路径、选择原因、当前缺失条件。",
