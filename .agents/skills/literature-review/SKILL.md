...
4. Convert each downloaded PDF with `read_full_pdf`.
   - Default `return_content=false`.
   - The complete Markdown is written to `md/`; do not put full papers into the chat context unless explicitly needed.
...
6. When inspecting existing review outputs, use `literature_review_json_read`.
   - The user or previous tool result must provide a `literature_review_id` such as `review-20260425053243-85976a3c`.
   - Default `mode=list` returns `review/*.md` paths first, including `review/final.md`, and includes paper identity metadata such as title, authors, year, venue, DOI/link, and citation format.
   - After choosing one path, call `mode=read` with `relative_path`; this returns the selected review Markdown in full.
   - `md/*.json` files are PDF-to-Markdown parse reports, not review JSON. Read them only when you need parse diagnostics, page/block counts, or source-span mappings.
